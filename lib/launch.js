 ...
 *   shared: {boolean} - DynamoDB will use a single database file, instead of using separate
 *                       files for each credential and region.
 *   heap: {?string=} - The amount of heap space, e.g., 512m. Uses JVM memory syntax.
 *                      If not specified, uses JVM defaults.
 *                      @see http://docs.oracle.com/javase/7/docs/technotes/tools/solaris/java.html
 * @typedef {Object}
 */

/**
 * A constructor that takes two basic arguments or a object with properties as its first argument
...
function launch(options, port) {
    // user has specified directory (persistent mode)
